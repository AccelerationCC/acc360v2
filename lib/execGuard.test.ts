...
describe('POST /api/invitations — the three steps, in order', () => {
  it('400s a malformed body (step 1) before consulting the session', async () => {
  ...
  it('401s an unauthenticated caller with a valid body (step 2)', async () => {
    const { POST } = await import('@/app/api/invitations/route')
    authMock.mockResolvedValue({ userId: null })
  ...
  it('403s an "hr" caller with a valid body (step 3), sending nothing', async () => {
    const { POST } = await import('@/app/api/invitations/route')
    signedInAs('hr')
    const res = await POST(inviteRequest({ emails: ['a@example.com'] }))
    expect(res.status).toBe(403)
